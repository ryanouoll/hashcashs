...
🌐 **線上 demo：** [hashcashs.pages.dev](https://hashcashs.pages.dev) · 🇺🇸 **[English README](./README.md)**
...
| | |
|---|---|
| **前端** | [hashcashs.pages.dev](https://hashcashs.pages.dev) |
...
這是 **demo / testnet 產品**。三個**故意保留**的簡化：
...
這些限制都在 `contracts/EmailVaultUSDC.sol` 註解寫清楚，在 `test/EmailVaultUSDC.test.js` 用**具名 test 釘住** — 任何人想修這些漏洞，那些 test 會失敗，逼他明確處理掉這份「故意保留」的承諾。
...
18 歲，台灣學生。第一版（純 ETH、無擁有權驗證）花 6 週 ship 出來。這個 USDC 版本是第 13 次迭代。

我親手測試的對象：媽媽（60 歲）、阿嬤（80 歲）、資工系同學。阿嬤跟媽媽都能用、資工同學說 UX 很爽。**接下來的目標**：證明有 10 家 web3 公司願意付錢用 hashcash 處理他們的非 crypto contractor 付款。
